...
class SebPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `seb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM seb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO seb (date, title, link, content)"
